...
const SourceMaterialModal = ({
  ...
  return <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className={cn("max-w-2xl w-[95vw] max-h-[80vh] flex flex-col overflow-hidden", "bg-surface border border-foreground rounded-xl", "shadow-offset-dark p-0")}>
        <DialogHeader className="flex-shrink-0 bg-surface border-b border-foreground/20 px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-coral/20 flex items-center justify-center">
              <BookOpen className="text-coral w-[20px] h-[20px]" />
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 pb-8 space-y-4">
          <AnimatePresence mode="wait">
            ...
            return <motion.div key={concept.id} ref={isActive ? activeRef : undefined} initial={{
              opacity: 0,
              y: 10
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.1
            }} className={cn("relative rounded-xl p-5 transition-all duration-300", isActive ? "bg-coral/15 border-2 border-coral shadow-offset-pink" : "bg-muted/30 border border-foreground/10 opacity-50 blur-[0.5px]")}>
                ...
                scale: 1
                  ...
                  {/* Concept title */}
                  <h3 className={cn("font-display font-semibold mb-3 transition-all", isActive ? "text-lg text-foreground" : "text-base text-muted-foreground")}>
                    {concept.title}
                  </h3>

                  {/* Concept content */}
                  <p className={cn("leading-relaxed transition-all", isActive ? "text-foreground" : "text-muted-foreground text-sm")}>
                    {concept.snippet}
                  </p>

                  {/* Hover overlay for inactive concepts */}
                  {!isActive && <div className="absolute inset-0 rounded-xl bg-transparent hover:bg-background/30 transition-all cursor-pointer group">
